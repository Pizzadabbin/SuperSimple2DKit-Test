...
public class NewPlayer : PhysicsObject {
    ...
    [Header("Sprint Data")]
    [SerializeField] private float _sprintMultipler;
    [SerializeField] private float _sprintInAirLoss;
    [Header("Wall Jump/Slide Data")]
    ...
    private float _wallJumpTimeCounter = 0;

    private float _sprintMultiplyerCounter = 0;
    ...
    private bool _doubleJumpPossile = true;
    private bool _waitedForOneFrame = false;
    protected void ComputeVelocity() {
        ...
        if(!frozen) {
            ...
            if(Input.GetButtonDown("Jump") && animator.GetBool("grounded") == true && !jumping) {
                ...
                Jump(1f);
                _waitedForOneFrame = true;
            }

            // double jump checking - Discord: derStax#0055                                                 // !!!!
            if(Input.GetButtonDown("Jump") && _doubleJumpPossile && !_waitedForOneFrame && !_isWallSliding) {
                animator.SetBool("pounded", false);
                Jump(1f);
                _doubleJumpPossile = false;
            }
            _waitedForOneFrame = false;
    ...
    // 8th January
    private void CalculateSprintVelocity(Vector2 move) {
        // Add smoothly sprint/jumping speed Discord: derStax#0055
        if(Input.GetKey(KeyCode.LeftShift) && !jumping) {
            _sprintMultiplyerCounter = _sprintMultipler;
        } else {
            if(!jumping) { 
                _sprintMultiplyerCounter -= _sprintInAirLoss * Time.deltaTime;
                if(_sprintMultiplyerCounter < 1) {
                    _sprintMultiplyerCounter = 1;
                }
            }
        }
        if(jumping) {
        ...
        targetVelocity = move * maxSpeed * _sprintMultiplyerCounter;
    }

    // 10th January
    private bool IsOnWall() { return Physics2D.OverlapCircle(  _wallCheck.position, 0.2f,   _wallLayer); }
    private bool IsOnGround() { return Physics2D.OverlapCircle(_groundCheck.position, 0.2f, _groundLayer); }
    private void WallSliding() {
        if(IsOnWall() && !IsOnGround() && Input.GetAxis("Horizontal") != 0f) {
            _isWallSliding = true;
            velocity.y = -_wallSlidingSpeed;
        } else {
            if(_isWallSliding) {
                velocity.y = -9.81f;
            }
    ...
    private void WallJumping() {
        if(_isWallSliding) {
            ...
            _wallJumpDirection = transform.localScale.x < 0 ? 1 : -1;
            _wallJumpTimeCounter = _wallJumpTime;
        } else {
            _wallJumpTimeCounter -= Time.deltaTime;
        }

        if(Input.GetButtonDown("Jump") && _wallJumpTimeCounter > 0f) {
            _isWallJumping = true;
            _hasWallJumped = true;
            Vector2 v2 = new Vector2(_wallJumpDirection * _wallJumpPower.x, _wallJumpPower.y);
            rb2d.AddForce(v2, ForceMode2D.Impulse);
